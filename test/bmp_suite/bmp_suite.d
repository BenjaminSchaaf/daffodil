...
void testGoodBMPImages(string fileName) {
    auto image = bmp.load!Pixel24Bpp("test/bmp_suite/g/" ~ fileName);
...
void testBadBMPImages(string fileName) {
    shouldThrow!(ImageException)(
        bmp.load!Pixel24Bpp("test/bmp_suite/b/" ~ fileName)
    );
}
...
void testQuestionableBMPImages(string fileName) {
    try {
        auto image = bmp.load!Pixel24Bpp("test/bmp_suite/q/" ~ fileName);
        assert(image !is null);
